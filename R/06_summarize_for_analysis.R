...
column_type <- main_summary %>% select(- column_type$column) %>% colnames() %>% data.frame(column = ., type = rep("response", length(.))) %>% rbind(column_type, .)
...
column_type <- main_summary %>% select(- column_type$column) %>% colnames() %>% data.frame(column = ., type = rep("predictor", length(.))) %>% rbind(column_type, .)
...
column_type <- main_summary %>% select(- column_type$column) %>% colnames() %>% data.frame(column = ., type = rep("category", length(.))) %>% rbind(column_type, .)
...
for (this_year in c(2020, 2021)) {
  temp_veg_vars <- vegetation_data %>% subset(year == this_year) %>% select(- year, - plot)
...
veg_summary$burned <- plot_data$calwood_fire_impact %>% vapply(FUN = function(x){if (x == "Burned") {return(1)} else {return(0)}}, FUN.VALUE = c(0))
